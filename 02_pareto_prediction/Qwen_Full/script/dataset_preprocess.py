...
- Feed the composition prompt directly to the Blablador embeddings endpoint to get a material embedding.
...
def _encode_elements_blablador(elements: List[str]) -> Dict[str, np.ndarray]:
    ...
    (Kept for compatibility; not used in the new composition embedding logic.)
    """
...
def matscibert_embed_and_similarity(  # name kept for API compatibility
    ...
    *,
    ...
    round_dp: int = 3,
    extra_tags: Optional[Dict[str, Union[str, int, float, bool]]] = None,
    batch_size: int = 64,                # kept for API compatibility (used for local batching)
    ...
    MATERIAL EMBEDDING LOGIC (Blablador backend, UPDATED):
    - Identify element columns (intersection with periodic table).
    - For each row, build a composition prompt via _row_to_prompt.
    - Feed the composition prompt directly to the Blablador embeddings endpoint.
    - Use the returned L2-normalized vector as the material embedding.
    ...
    # ---- 1) Build composition prompts per row ----
    ...
    if row_prompts:
        for start in range(0, len(row_prompts), batch_size):
            ...
            batch_embs = _encode_texts_blablador(batch_texts)  # [B, d], L2-normalized
